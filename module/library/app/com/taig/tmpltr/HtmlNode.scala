...
abstract class	HtmlNode[N]( label: String, minimizeEmpty: Boolean, child: NodeSeq, val attrs: Attributes )
extends 		Elem( null, label, Null, TopScope, minimizeEmpty, child: _* )
{
	def this( element: Elem, attributes: Attributes = Attributes.empty ) =
	{
		this( element.label, element.minimizeEmpty, element.child, new Attributes( attributes ++ element.attributes.asAttrMap ) )
	}
